...
public class MemberServiceImpl implements MemberService {
	...
	public void memberJoin(HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			...
			MemberVO member = dao.getMemberById(id);	// 아이디 중복 검사
			if (member != null) {
	...
	public void findPassSubmit(HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			...
			if (!isCheck) {
				System.out.println("일치하는 사용자 정보가 없음");
				throw new NullPointerException("오류 발생 : 일치하는 사용자 정보가 없음");
			}
			
			// 사용자 정보 존재 확인, code를 사용자의 이메일에 전송
			...
			InternetAddress fromAddress = new InternetAddress("master@koreate.net");
			msg.setSentDate(new Date());
			msg.setHeader("Content-Type", "text/html;charset=utf-8;");
			msg.setRecipient(Message.RecipientType.TO, to);
			msg.setFrom(fromAddress);
			
			msg.setSubject("비밀번호 찾기 코드입니다.", "UTF-8");
			...
			mail.append("<form action='http://192.168.1.47:8080");
			...
			e.printStackTrace();
			pw.print("<script>");
			pw.print("alert('서비스에 문제가 있습니다. 다시 이용해주세요." + e.getMessage() + "');");
			pw.print("location.href='login.mc';");
			pw.print("</script>");
		}

...
<s:query var="r" dataSource="jdbc/MySQLDBCP">
	SELECT count(*) AS cnt FROM big_member WHERE u_id != 'admin' AND u_join = 'Y' ORDER BY u_num DESC
</s:query>

<c:set target="${pm }" property="totalCount" value="${r.rows[0].cnt }" />

<%-- <s:query var="rs" dataSource="jdbc/MySQLDBCP">
	SELECT * FROM big_member WHERE u_id != 'admin' AND u_join = 'Y' ORDER BY u_num DESC limit ${pm.cri.startRow }, ${pm.cri.perPageNum }
</s:query> --%>

<s:query var="rs" dataSource="jdbc/MySQLDBCP" startRow="${pm.cri.startRow }" maxRows="${pm.cri.perPageNum }">
	SELECT * FROM big_member WHERE u_id != 'admin' AND u_join = 'Y' ORDER BY u_num DESC
</s:query>
...
<c:choose>
	...
	<tr>
		<td>${row.u_num }</td>
		<td>${row['u_id'] }</td>
		<td><f:formatDate value="${row.u_regdate }" pattern="yyyy.MM.dd hh:mm" /></td>
		<td>
